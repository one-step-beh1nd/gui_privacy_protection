...
# Lock for port allocation to prevent race conditions
...
def find_free_ports(start_port=6060):
    ...
    Uses a lock to prevent race conditions when multiple threads allocate ports simultaneously.
    """
    ...
    with _port_lock:
        ...
        while attempts < max_attempts:
            # Check if port is free both in system and in our allocated set
            if port not in _allocated_ports and is_port_free(port):
                _allocated_ports.add(port)
                return port
            port += 1
            attempts += 1
        raise Exception(f"Could not find a free port after {max_attempts} attempts starting from {start_port}")


def release_port(port):
    """
    Release a port from the allocated set.
