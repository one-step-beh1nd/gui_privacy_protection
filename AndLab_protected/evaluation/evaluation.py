...
def _with_privacy_notice(instruction: str) -> str:
    """Task text unchanged; screenshot privacy is in SoM system prompts (DualTAP, no token placeholders)."""
    return instruction
...
class AutoTask():
    ...
    def run_step(self, round_count):
        ...
        try:
            ...
            if compressed_xml_json is None:
                compressed_xml_json = "[XML fetch failed: Unable to retrieve UI hierarchy. Element-based operations (tap, long_press, swipe) are not available. Please use coordinate-based operations or wait and retry.]"
