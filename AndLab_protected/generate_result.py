...
def parse_args():
    ...
    group.add_argument("--use_screenshot_judge", action="store_true", default=False,
                       help="Use LLM vision model to judge task completion by final screenshot (for operation type tasks)")
    group.add_argument("--target_dirs", type=str, nargs="+", default=None,
                       help="Specific directories to evaluate. If not specified, all directories will be evaluated.")
...
def main():
    ...
    all_input_dirs = [os.path.join(input_folder, input_dir) for input_dir in os.listdir(input_folder)]
    
    # Filter input directories if target_dirs is specified
    if args.target_dirs:
        input_dirs = [input_dir for input_dir in all_input_dirs 
                     if os.path.basename(input_dir) in args.target_dirs]
        if not input_dirs:
            print(f"Warning: No matching directories found in {input_folder} for {args.target_dirs}")
            return
        print(f"> Will evaluate only: {[os.path.basename(d) for d in input_dirs]}")
    else:
    ...
    for input_dir in input_dirs[:]:  # Use slice copy to avoid modification during iteration
