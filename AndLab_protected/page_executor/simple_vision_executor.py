...
def traverse_tree(xml_path, elem_list, attrib, add_index=False):
    ...
    if xml_path is None or xml_path == "ERROR":
        raise ValueError(f"Invalid XML path: {xml_path}")
...
class VisionExecutor(TextOnlyExecutor):
    ...
    def set_elem_list(self, xml_path):
        # 验证xml_path是否有效
        if xml_path is None or xml_path == "ERROR" or not os.path.exists(xml_path):
            raise ValueError(f"Invalid XML path: {xml_path}")
        clickable_list = []
        focusable_list = []
    ...
    def tap(self, index):
        if not hasattr(self, 'elem_list') or len(self.elem_list) == 0:
            error_msg = f"Cannot tap: element list is empty (XML fetch may have failed). Available elements: 0, requested index: {index}"
            raise ValueError(error_msg)
        if index <= 0 or index > len(self.elem_list):
            error_msg = f"Tap Index {index} out of range. Available elements: {len(self.elem_list)}"
    ...
    def long_press(self, index):
        if not hasattr(self, 'elem_list') or len(self.elem_list) == 0:
            error_msg = f"Cannot long_press: element list is empty (XML fetch may have failed). Available elements: 0, requested index: {index}"
            raise ValueError(error_msg)
        if index <= 0 or index > len(self.elem_list):
            error_msg = f"Long Press Index {index} out of range. Available elements: {len(self.elem_list)}"
            raise IndexError(error_msg)
        tl, br = self.elem_list[index - 1].bbox
        x, y = (tl[0] + br[0]) // 2, (tl[1] + br[1]) // 2
    ...
    def swipe(self, index, direction, dist):
        if not hasattr(self, 'elem_list') or len(self.elem_list) == 0:
            error_msg = f"Cannot swipe: element list is empty (XML fetch may have failed). Available elements: 0, requested index: {index}"
            raise ValueError(error_msg)
        if index <= 0 or index > len(self.elem_list):
            error_msg = f"Swipe Index {index} out of range. Available elements: {len(self.elem_list)}"
            raise IndexError(error_msg)
        tl, br = self.elem_list[index - 1].bbox
        x, y = (tl[0] + br[0]) // 2, (tl[1] + br[1]) // 2
